...
const STORAGE_KEYS: { [id: string]: string } = {
  ...
  FIRST_TIME_USER_EXPERIENCE_ID: "FIRST_TIME_USER_EXPERIENCE_ID",
};
...
export const setFirstTimeUserExperienceApplicationId = async (id: string) => {
  try {
    await store.setItem(STORAGE_KEYS.FIRST_TIME_USER_EXPERIENCE_ID, id);
    return true;
  } catch (error) {
    log.error("An error occurred while setting FIRST_TIME_USER_EXPERIENCE_ID");
    log.error(error);
  }
};
...
export const getFirstTimeUserExperienceApplicationId = async () => {
  try {
    const id = await store.getItem(STORAGE_KEYS.FIRST_TIME_USER_EXPERIENCE_ID);
    return id;
  } catch (error) {
    log.error("An error occurred while fetching FIRST_TIME_USER_EXPERIENCE_ID");
    log.error(error);
  }
};
